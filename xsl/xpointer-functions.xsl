<?xml version="1.0" encoding="utf-8"?>
	<!--
		Copyright (c) 2002 ISOGEN International This style sheet fragment
		implements the resolution of xpointers. It requires implementation of
		the exslt common and functions modules. Author: W. Eliot Kimber,
		eliot@isogen.com $Revision: 1.1 $
	-->
	<!-- Status:
...
<xsl:stylesheet version="1.0"
	...
	<func:function name="xptrf:resolve-xpointer-url">
		<!--
			Given an element that exhibits an href attribute, attempts to resolve
			the URL and XPointer (if present) into a result node list. If there
			is no fragment identifier, acts as though the fragment identifier
			"#/" had been specified, returning the document root.
		-->
	...
	<func:function name="xptrf:resolve-xpointer">
		<!--
			Resolves an xpointer in the context of some location source node. The
			location source is either the pointer, if the URL was just an
			XPointer, or it's the document element of the document addressed by
		...
		<xsl:for-each select="$location-source-node">
			<!--
				Setting the context to the pointer node so that relative URLs are
				resolved relative to the pointer node by saxon:evaluate()
			-->
			<xsl:choose>
				<xsl:when test="$xpointer != ''">
					...
					<xsl:choose>
						<xsl:when test="string($direct-result-set) = ''">
							<xsl:message>
								XIndirect warning: XPointer "
								<xsl:value-of select="$xpointer" />
								" did not address any nodes.
							</xsl:message>
						</xsl:when>
				...
				<xsl:otherwise>
					<xsl:message>
						XPointer error: $xpointer value is '' in resolve-xpointer.
					</xsl:message>
					<func:result select="/.." />
				</xsl:otherwise>
			</xsl:choose>
	...
	<func:function name="xptrf:fragid2xpointer">
		...
		-->
		<!--
			NOTE: does not: - Handle multi-part XPointers:
			"#xpointer(foo)xpointer(bar)" - Skip non-xpointer schemes Doing this
			would require more sophisticated string processing than I can
...
'^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^'),
	...
	<func:function name="xptrf:construct-child-sequence">
		...
		<xsl:variable name="rest"
			select="substring-after($xptr-child-seq, '/')" />
		<xsl:choose>
			<xsl:when test="$rest = ''">
			...
			<xsl:otherwise>
				<func:result
					select="xptrf:construct-child-sequence(
	...
	<func:function name="xptrf:xpointer-error">
		...
		<xsl:param name="fragid" />
		<xsl:message>
			XPointer error: fragment identifier "
			<xsl:value-of select="$fragid" />
			" is not a valid XPointer.
			Returning "/.." as XPath to resolve (empty node set)
		</xsl:message>
