...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="xinclude-copy-attributes">
		<!--
			Implement this template in the main template to specialize to a
	...
	<xsl:output method="xml" indent="yes" doctype-system="foo"
		omit-xml-declaration="no" encoding="UTF-8" />

	<!--
		==============================================================
		XInclude implementation Implements XInclude by processing the entire
		doc to produce a single result tree with all the includes resolved and
		then applies the normal template processing to that document.
		==============================================================
	-->
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="//xi:include">
				...
				<xsl:apply-templates select="$resolved-doc"
					mode="normal" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates />
	...
	<xsl:template match="xi:include" mode="xinclude">
		...
		<xsl:choose>
			<xsl:when test="$xpath != ''">
				...
				<xsl:apply-templates select="xptrf:resolve-xpointer-url(.)"
					mode="xinclude" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:message>
					Xinclude: Failed to get a value for the href= attribute
					of xi:include element.
				</xsl:message>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
